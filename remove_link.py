...
def main():
    ...
    config_file_name = config_file_name.split('.')  # Get the file name without the extension
    ...
    if account_data:
        ...
        # Get link IDs from bio_links
        link_ids = user_instagram.post_get_links_data(user_id=account_data['data']['id'])
        logging.info(f"Retrieved link IDs: {link_ids}")

        # Remove bio links
        if link_ids:
            result = user_instagram.remove_bio_links(link_ids)
            logging.info(f"Result of removing links: {result}")
        else:
            logging.info("No bio links found to remove.")
